...
<div class="admin-data-container container-fluid d-block justify-content-center">
    <h3 >Quản lý số liệu</h3>
    <hr>

    <div class="charts">
        <div class="chart-container">
            <h3>Lượt mua hôm nay</h3>
            <canvas id="chart1" class="data-chart scalesmall"></canvas>
        </div>

        <div class="chart-container">
            <h3>Lượt mua 7 ngày gần nhất</h3>
            <canvas id="chart2" class="data-chart"></canvas>
        </div>

        <div style="margin-top: -100px;" class="chart-container fullwidth-chart">
            <h3>Lượt mua trong tháng</h3>
...
<script>
    ...
    $(document).ready(()=>{
        $.getJSON(window.location.protocol + '//' + window.location.host + '/api/v1/getdata').done((data) => {
            week = []
            for (let i = -6; i <= 0; i++) {
                day = new Date()
